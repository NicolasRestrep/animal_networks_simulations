...
net <- read_rds("~/Documents/diffusion_animal_networks/Data/networks/elph_w2_age.rds")
...
rugged_crawl_vertical <- function(r_max,
  ...
  for (j in 1:sims) {
    ...
    if (removal == TRUE) {
      ...
      ntbr <-  sample(1:vcount(net), nrems)
...
saveRDS(df,
        "~/Documents/diffusion_animal_networks/Results/ew2_rand_all.rds")
